...
func NewAuthHandler(next http.Handler, aggrRoute *models.AggregateRoute, httpClient *http.Client, store sessions.Store) *AuthHandler {
	...
	if aggrRoute.Auth.OIDCAuth != nil && aggrRoute.Auth.OIDCAuth.Endpoint != "" {
...
func (a AuthHandler) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if a.aggrRoute.Auth.OIDCAuth != nil && a.aggrRoute.Auth.OIDCAuth.Endpoint != "" && req.URL.Path == a.aggrRoute.Auth.OIDCAuth.AuthPath {
		a.redirectToOIDC(w, req)
		return
	}

	if a.aggrRoute.Auth.OIDCAuth != nil && a.aggrRoute.Auth.OIDCAuth.Endpoint != "" && req.URL.Path == a.aggrRoute.Auth.OIDCAuth.CallbackPath {
	...
	if jwtToken == "" {
		if a.aggrRoute.Auth.OIDCAuth != nil && a.aggrRoute.Auth.OIDCAuth.Endpoint != "" {
